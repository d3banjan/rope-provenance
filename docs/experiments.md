...
| `slm-qwen25-0.5b-instruct-source-operation-rail-s0` | blocked | CLI prepared: `scripts/slm_policy_rail.py --steps 300 --eval-every 100 --train-pairs 1024 --eval-pairs 64 --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --fail-on-truncation` | `results/slm/qwen25_0_5b_instruct_source_operation_rail_s0.json` | none | source+operation embedding checkpoint planned | Rung 2 source+operation oracle rail. Harness selfcheck passed with `steps=0` at `results/slm/qwen25_0_5b_instruct_source_operation_rail_selfcheck_s0.json`. Scored GPU run blocked on 2026-05-22 by active external `lean-mining` LFS5 process using ~10.2 GB VRAM. |
