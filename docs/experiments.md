...
| `toy-gate-pretrain-syntax-reg-bs512-s0` | planned | CLI: `--template-mode gate_pretrain --gate-kinds no_not question --vocab-template-modes gated --hide-tags --role-control correct --block-size 512 --batch-size 512 --answer-loss-weight 4 --weight-decay 0.1 --dropout 0.05 --embedding-dropout 0.05 --label-smoothing 0.02 --fail-on-truncation` | `results/toy/toy_gate_pretrain_syntax_reg_bs512_s0.json` | planned | result JSON only | Cheap scratch kill-test after semantic gate failed. Expected: maybe easier than semantic gate, but still weak. Pass gate: exact-match SEP >= 0.50; kill scratch gated branch if final SEP < 0.10. |
...
| `slm-qwen25-0.5b-base-gate-pretrain-s0` | planned | pending script/config | `results/slm/qwen25_0_5b_base_gate_pretrain_s0.json` | planned | adapter/checkpoint plus result JSON | Capability prerequisite before hidden roles. Passing threshold: heldout exact-match >= 0.50 and coherent copied outputs; if it fails, hidden-role additive Qwen runs remain blocked. |
