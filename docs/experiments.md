...
| `slm-qwen25-0.5b-base-gate-pretrain-answer-s0` | completed | CLI: same script/model with `--prompt-format answer --batch-size 16 --eval-batch-size 16 --lora-rank 8 --lr 2e-4` | `results/slm/qwen25_0_5b_base_gate_pretrain_answer_s0.json` | `g1ewvphj` | adapter checkpoint ignored, result JSON tracked | Failed. Exact-match fell from 0.047 at step 1 to 0.000 after train loss saturated; samples are degenerate numbered/repeated outputs. Base hidden-role runs remain blocked. |
...
| `slm-qwen25-0.5b-instruct-chat-zero-s0` | planned | CLI: same script with `--model /mnt/expansion/huggingface/hub/models--Qwen--Qwen2.5-0.5B-Instruct/snapshots/7ae557604adf67be50417f59c2c2f167def9a775 --prompt-format chat --steps 0` | `results/slm/qwen25_0_5b_instruct_chat_zero_s0.json` | planned | result JSON tracked | Confounded eval-only capability probe. If exact-match is nontrivial, the task is reachable via instruction post-training; if it is near zero, run one short instruct LoRA chat fine-tune before redesigning data. |
| `slm-qwen25-0.5b-instruct-gated-probe-s0` | blocked | pending after zero-shot instruct | pending | planned | SEP JSON | Confounded fine-tune probe only. If it works, it shows the task is within reach of an instruction-tuned SLM, not that the provenance channel caused the behavior. |
