...
| `slm-qwen25-0.5b-instruct-gated-probe-shift-s0` | planned | same script/model with corrected next-token loss, `--prompt-format chat --steps 500 --batch-size 16 --eval-batch-size 16 --lora-rank 8 --lr 2e-4` | `results/slm/qwen25_0_5b_instruct_gate_pretrain_shift_s0.json` | planned | adapter checkpoint ignored, result JSON tracked | Confounded reachability check only. Passing threshold: exact-match >=0.50. If it fails, redesign the generator/loss before hidden-role work; if it passes, the task is reachable with instruction-posttraining priors but not causal evidence for provenance. |
