...
| `slm-qwen25-0.5b-base-gate-pretrain-answer-shift-s0` | planned | same script/model with corrected next-token loss and `--prompt-format answer --batch-size 16 --eval-batch-size 16 --lora-rank 8 --lr 2e-4` | `results/slm/qwen25_0_5b_base_gate_pretrain_answer_shift_s0.json` | planned | adapter checkpoint ignored, result JSON tracked | Valid Qwen base capability rerun. Passing threshold remains exact-match >=0.50 with coherent copied outputs. |
