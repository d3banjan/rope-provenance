...
| `slm-qwen25-0.5b-instruct-hidden-role-strict-audit-s0` | planned | eval-only reload of the three saved hidden-role adapters with strict normalized answer equality | `results/slm/qwen25_0_5b_instruct_hidden_role_*_strict_eval_s0.json` | planned | result JSON tracked | First falsification gate. Kill the 1.000 claim if strict correct drops materially, constant exceeds 0.55, or eval-swap exceeds 0.10. |
...
| `slm-qwen25-0.5b-instruct-hidden-role-subspace-s0` | planned | `scripts/analyze_slm_adapter_subspace.py` over saved adapters | `results/slm/qwen25_0_5b_instruct_hidden_role_subspace_s0.json` | planned | JSON tracked | Measures LoRA stable rank and overlap/capture against cached Qwen base-to-instruct deltas. |
