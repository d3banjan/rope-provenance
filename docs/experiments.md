...
| `slm-qwen25-0.5b-instruct-hidden-role-gated-evalswap-s0` | planned | same as correct hidden-role run but `--eval-role-control swap_instr_data --steps 500` | `results/slm/qwen25_0_5b_instruct_hidden_role_gated_evalswap_s0.json` | planned | adapter checkpoint ignored, result JSON tracked | Directionality check after the positive correct/constant pair. Pre-registered expectation: exact-match <=0.10 because eval roles point to the opposite valid candidate while labels remain unchanged. |
