...
| `slm-qwen25-0.5b-instruct-hidden-role-gated-correct-s0` | planned | same instruct model with `--template-mode gated --hide-tags --prompt-format chat --input-role-embeddings --role-control correct --steps 500 --batch-size 16 --eval-batch-size 8` | `results/slm/qwen25_0_5b_instruct_hidden_role_gated_correct_s0.json` | planned | adapter checkpoint ignored, result JSON tracked | Confounded hidden-role proof-of-concept. Pre-registered pass: exact-match >=0.80 and later margin >=+0.30 over constant-role control. Batch smoke peaked at 8.16GB. |
