...
| `toy-add-role-hidden-swapped-s0` | deferred | CLI: same toy script with `--hide-tags --role-control swap_instr_data --answer-loss-weight 4 --batch-size 1024` | `results/toy/role_embedding_hidden_tags_swapped_bs1024_s0.json` | planned | result JSON only | Deferred because the simple-template constant-role shortcut already made this generator non-diagnostic. |
...
| `toy-add-role-hidden-gated-constant-s0` | blocked | CLI: `--template-mode gated --hide-tags --role-control constant --answer-loss-weight 4 --batch-size 1024` | `results/toy/role_embedding_hidden_tags_gated_constant_bs1024_s0.json` | planned | result JSON only | Blocked with the primary scratch gated-role run; the scratch syntactic gate stayed below threshold. |
| `toy-add-role-hidden-gated-evalswap-s0` | blocked | CLI: `--template-mode gated --hide-tags --role-control correct --eval-role-control swap_instr_data --answer-loss-weight 4 --batch-size 1024` | `results/toy/role_embedding_hidden_tags_gated_traincorrect_evalswap_bs1024_s0.json` | planned | result JSON only | Blocked with the primary scratch gated-role run; no need to spend toy GPU until the scratch prerequisite is redesigned. |
...
| `slm-qwen25-0.5b-instruct-hidden-role-gated-correct-s0` | completed | same instruct model with `--template-mode gated --hide-tags --prompt-format chat --input-role-embeddings --role-control correct --steps 500 --batch-size 16 --eval-batch-size 8` | `results/slm/qwen25_0_5b_instruct_hidden_role_gated_correct_s0.json` | `hdjhcz4q` | adapter checkpoint ignored, result JSON tracked | Confounded hidden-role proof-of-concept. Final exact-match 1.000 from step 100 onward; samples alternate correctly between witness and answer for identical visible prompts. Constant-role control still required. |
| `slm-qwen25-0.5b-instruct-hidden-role-gated-constant-s0` | completed | same as correct hidden-role run but `--role-control constant` | `results/slm/qwen25_0_5b_instruct_hidden_role_gated_constant_s0.json` | `rldphw35` | adapter checkpoint ignored, result JSON tracked | Required text-only contradiction control. Final exact-match 0.500, matching the paired-prompt contradiction ceiling. Correct-minus-constant margin is +0.500. |
| `slm-qwen25-0.5b-instruct-hidden-role-gated-evalswap-s0` | completed | same as correct hidden-role run but `--eval-role-control swap_instr_data --steps 500` | `results/slm/qwen25_0_5b_instruct_hidden_role_gated_evalswap_s0.json` | `u9uht6a8` | adapter checkpoint ignored, result JSON tracked | Directionality check after the positive correct/constant pair. Final exact-match 0.000; samples show each output flips to the opposite valid candidate under swapped roles. |
| `slm-qwen25-0.5b-instruct-hidden-role-strict-audit-s0` | completed | eval-only reload of the three saved hidden-role adapters with strict normalized answer equality | `results/slm/qwen25_0_5b_instruct_hidden_role_*_strict_eval_s0.json` | `qu1d8i4g`, `e54lk3vo`, `1w49y3cg` | result JSON tracked | Passed the eval-artifact kill test. Strict exact: correct 1.000, constant 0.500, eval-swap 0.000. |
| `slm-qwen25-0.5b-instruct-hidden-role-roleonly-s0` | completed | same hidden-role gated setup with `--input-role-embeddings --lora-rank 0`, all gate kinds by default | `results/slm/qwen25_0_5b_instruct_hidden_role_roleonly_s0.json` | `d8icto6q` | role embedding checkpoint ignored, result JSON tracked | Strong kill of the LoRA-necessity thesis. With only 3,584 trainable role-vector params, strict exact reached 0.996 on the all-gate mix. Active role rows stable-rank 2.89. |
| `slm-qwen25-0.5b-instruct-hidden-role-roleonly-controls-s0` | completed | reload correct role-only checkpoint with constant and eval-swapped roles; separately train constant role-only | `results/slm/qwen25_0_5b_instruct_hidden_role_roleonly_*_s0.json` | `f34o4v4u`, `s0t5jxbt`, `n5i8ped4` | result JSON tracked | Directionality/control check for role-only. Eval-time constant 0.102, eval-swap 0.000, trained constant 0.438. |
| `slm-qwen25-0.5b-base-hidden-role-roleonly-smoke-s0` | completed | Qwen2.5-0.5B base with `--prompt-format answer --input-role-embeddings --lora-rank 0 --steps 200`, all gate kinds | `results/slm/qwen25_0_5b_base_hidden_role_roleonly_smoke_s0.json` | `q3eoikyp` | role embedding checkpoint ignored, result JSON tracked | Base/instruct split kill test. Base reaches only strict exact 0.156 at step 200, while instruct role-only reached 0.980 at step 100. Supports the claim that SFT exposes the usable role-control surface. |
| `slm-qwen25-0.5b-instruct-source-rail-s0` | completed | CLI: `scripts/slm_source_rail.py --steps 300 --eval-every 100 --train-pairs 1024 --eval-pairs 64 --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --fail-on-truncation` | `results/slm/qwen25_0_5b_instruct_source_rail_s0.json` | none | source embedding checkpoint ignored, result JSON tracked | First policy-IR Rung 1 smoke. Trains only 6 x 896 source embeddings. Correct source rail strict exact 1.000; trusted-follow 1.000; untrusted-suppress 1.000. Constant-source eval 0.305 and trusted/untrusted source-swap eval 0.000. Synthetic source-only positive; not yet source+operation composition. |
| `slm-qwen25-0.5b-instruct-source-operation-rail-s0` | completed | CLI: `scripts/slm_policy_rail.py --steps 300 --eval-every 100 --train-pairs 1024 --eval-pairs 64 --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --fail-on-truncation` | `results/slm/qwen25_0_5b_instruct_source_operation_rail_s0.json` | none | source+operation embedding checkpoint ignored, result JSON tracked | Rung 2 source+operation oracle rail. Trains only source+operation embeddings, 9,856 params. Final exact 1.000 across trusted OBEY, untrusted OBEY suppression, DATA USE, and DATA QUOTE. Controls: constant source 0.059, constant operation 0.215, source-swap 0.000, operation-swap 0.438. |
| `slm-qwen25-0.5b-instruct-policy-vector-s0` | completed / void | CLI: `scripts/slm_policy_vector.py --steps 300 --eval-every 100 --train-pairs 512 --eval-pairs 32 --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --fail-on-truncation` | `results/slm/qwen25_0_5b_instruct_policy_vector_s0.json` | none | policy-vector embedding checkpoint ignored, result JSON tracked | Rung 3 explicit policy vector. Source and operation rails remain oracle, but policy is supplied as OBEY/USE/QUOTE bits. Trains 15,232 params, no LoRA. Final exact 0.048, seen-policy exact 0.056, held-out policy 0.000 after loss 12.02 -> 1.96. Classify as `VOID / installation_failed`, not `NO_GENERALIZATION`, because seen policies did not install. |
| `slm-qwen25-0.5b-instruct-policy-vector-overfit-mask001-s0` | completed | CLI: `scripts/slm_policy_vector.py --steps 800 --train-pairs 8 --train-policy-masks 001 --eval-on-train --lr 1e-3 --weight-decay 0` | `results/slm/qwen25_0_5b_instruct_policy_vector_overfit_mask001_s0.json` | none | adapter ignored, result JSON tracked | Fixed-policy overfit diagnostic. Exact reaches 1.000 on the 24 training rows, so the harness/copy task and embedding-only fixed-policy capacity are not broken. |
| `slm-qwen25-0.5b-instruct-policy-vector-overfit-allseen-s0` | completed | CLI: `scripts/slm_policy_vector.py --steps 1200 --train-pairs 4 --eval-on-train --lr 1e-3 --weight-decay 0` plus eval-only reloads on fresh seen and held-out rows | `results/slm/qwen25_0_5b_instruct_policy_vector_overfit_allseen_s0.json`, `results/slm/qwen25_0_5b_instruct_policy_vector_overfit_allseen_evalseen64_s0.json`, `results/slm/qwen25_0_5b_instruct_policy_vector_overfit_allseen_evalheldout101_64_s0.json` | none | adapter ignored, result JSON tracked | All-seen-mask tiny overfit reaches 1.000 on 72 training rows, but reload eval drops to 0.261 on fresh seen rows and 0.219 on fresh held-out `101`. This shows memorization, not reusable `policy[operation]` binding. |
| `slm-qwen25-0.5b-instruct-permission-rail-s0` | completed | CLI: `scripts/slm_policy_vector.py --steps 300 --permission-rail oracle --no-policy-bit-embeddings --train-pairs 512 --eval-pairs 32 --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --fail-on-truncation` | `results/slm/qwen25_0_5b_instruct_permission_rail_s0.json` | none | permission/source/operation embedding checkpoint ignored, result JSON tracked | Deterministic binder rail. Software computes local ALLOWED/DENIED from `policy_bits[operation]`; model trains 12,544 params. Exact reaches 1.000 by step 200 and final exact is 1.000 on both seen policies and held-out `101`. |
| `slm-qwen25-0.5b-instruct-permission-only-s0` | completed | CLI: `scripts/slm_policy_vector.py --steps 300 --permission-rail oracle --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --train-pairs 512 --eval-pairs 32 --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --fail-on-truncation` | `results/slm/qwen25_0_5b_instruct_permission_only_s0.json` | none | permission embedding checkpoint ignored, result JSON tracked | Minimal compiled-permission rail. Only the 3 x 896 local permission embedding is trainable (2,688 params), no LoRA and no source/operation/raw-policy embeddings. Final exact is 1.000 on seen policies and held-out `101`, with every OPEN/DECLINE primitive cell at 1.000. This is the current 100% working ladder point. |
| `slm-qwen25-0.5b-instruct-pr4-permission-grid-s0` | completed | CLI: `scripts/slm_policy_vector.py --dataset-kind source_policy_grid --steps 300 --permission-rail oracle --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --train-pairs 512 --eval-pairs 8 --eval-controls correct constant_policy swap_policy --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --fail-on-truncation`; strict trap reload: `--steps 0 --load-adapter results/slm/qwen25_0_5b_instruct_pr4_permission_grid_s0.pt --eval-controls correct constant_policy invert_policy` | `results/slm/qwen25_0_5b_instruct_pr4_permission_grid_s0.json`, `results/slm/qwen25_0_5b_instruct_pr4_permission_grid_strict_trap_s0.json` | none | permission embedding checkpoint ignored, result JSON tracked | PR4 4-cell source-policy x template grid. Same 2,688-param permission-only rail reaches exact 1.000 on C1/C2/C3/C4, all OPEN/DECLINE primitive cells 1.000. Constant-policy trap 0.444; stricter invert-policy trap 0.000. The old OBEY/USE swap trap is weak on this grid because QUOTE is unchanged and lands at 0.667; use invert as the decisive trap. |
| `slm-qwen25-0.5b-instruct-pr5-sep-projection-eval-s0` | completed / early-kill | CLI: `scripts/slm_policy_vector.py --dataset-kind sep_projection --sep-json /tmp/sep_repo/SEP_dataset/SEP_dataset.json --steps 0 --eval-pairs 200 --eval-controls correct constant_policy invert_policy --permission-rail oracle --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --load-adapter results/slm/qwen25_0_5b_instruct_pr4_permission_grid_s0.pt --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --fail-on-truncation` | `results/slm/qwen25_0_5b_instruct_pr5_sep_projection_eval_s0.json` | none | eval-only result JSON tracked | PR5 benchmark-surface projection smoke. Denied SEP-style OBEY injection suppression reaches exact 0.900 on 200 rows, below the >=0.95 transfer gate. Invert-policy drops to 0.465, so the rail remains causal, but synthetic PR4 does not fully transfer to SEP surfaces without adaptation. Do not proceed to PR6 until PR5b or a redesigned benchmark projection closes this gap. |
| `slm-qwen25-0.5b-instruct-pr5b-sep-paired-pre-adapt-s0` | completed / baseline | CLI: `scripts/slm_policy_vector.py --dataset-kind sep_paired --sep-json /tmp/sep_repo/SEP_dataset/SEP_dataset.json --steps 0 --train-pairs 128 --eval-pairs 100 --eval-controls correct constant_policy invert_policy --permission-rail oracle --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --load-adapter results/slm/qwen25_0_5b_instruct_pr4_permission_grid_s0.pt --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --fail-on-truncation` | `results/slm/qwen25_0_5b_instruct_pr5b_sep_paired_pre_adapt_s0.json` | none | eval-only result JSON tracked | PR5b pre-adaptation baseline. Paired SEP surfaces have identical visible text with hidden `DENIED -> ANSWER` or `ALLOWED -> witness`. PR4 adapter reaches exact 0.395, OPEN_OBEY 0.040, DECLINE_OBEY 0.750, invert-policy 0.230. Confirms PR5b is not already solved before adaptation. |
| `slm-qwen25-0.5b-instruct-pr5b-sep-paired-s0` | completed | CLI: `scripts/slm_policy_vector.py --dataset-kind sep_paired --sep-json /tmp/sep_repo/SEP_dataset/SEP_dataset.json --steps 300 --eval-every 100 --train-pairs 512 --eval-pairs 200 --eval-controls correct constant_policy invert_policy --permission-rail oracle --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --batch-size 8 --grad-accum 2 --eval-batch-size 8 --max-new-tokens 16 --lr 2e-4 --fail-on-truncation --load-adapter results/slm/qwen25_0_5b_instruct_pr4_permission_grid_s0.pt` | `results/slm/qwen25_0_5b_instruct_pr5b_sep_paired_s0.json` | none | permission embedding checkpoint ignored, result JSON tracked | PR5b SEP-surface adaptation. Held-out paired SEP exact reaches 1.000, OPEN_OBEY 1.000, DECLINE_OBEY 1.000. Constant-policy control is 0.500 and invert-policy control is 0.000. Unblocks PR6 learned operation detection, with the caveat that benchmark-surface transfer required adaptation. |
| `slm-qwen25-0.5b-instruct-pr6-operation-detector-s0` | completed / early-kill | CLI: `scripts/slm_operation_detector.py --train-pairs 96 --eval-pairs 32 --steps 500 --feature-batch-size 16 --probe-batch-size 128` | `results/slm/qwen25_0_5b_instruct_pr6_operation_detector_s0.json` | none | result JSON tracked | PR6 frozen-Qwen operation-detector preflight. Linear probe over candidate-span hidden states reaches train accuracy 1.000 but eval accuracy 0.807. Seen-template cells C1/C3 are 1.000; held-out-template cells C2/C4 are 0.615. Shuffled-label trap is 0.380. Early kill under the >=0.95 detector gate; do not wire predicted operation ids into the rail stack yet. |
| `slm-qwen25-0.5b-instruct-pr7-tiny-binder-s0` | completed | CLI: `scripts/slm_policy_vector.py --dataset-kind policy_vector --steps 300 --eval-every 100 --train-pairs 512 --eval-pairs 32 --eval-controls correct constant_policy invert_policy --permission-rail binder --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --lr 2e-4 --fail-on-truncation` | `results/slm/qwen25_0_5b_instruct_pr7_tiny_binder_s0.json` | none | binder checkpoint ignored, result JSON tracked | PR7 learned policy binder. Tiny MLP over `[policy_bits, operation_onehot]` emits the local rail vector. Trainable params 29,792. Final exact 1.000, seen 1.000, held-out `101` 1.000, all OPEN/DECLINE primitive cells 1.000. Constant-policy control 0.429; invert-policy control 0.000. |
| `slm-qwen25-0.5b-instruct-pr7-binder-grid-s0` | completed / boundary | CLI: `scripts/slm_policy_vector.py --dataset-kind source_policy_grid --steps 300 --eval-every 100 --train-pairs 512 --eval-pairs 8 --eval-controls correct constant_policy invert_policy --permission-rail binder --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --lr 2e-4 --fail-on-truncation` | `results/slm/qwen25_0_5b_instruct_pr7_binder_grid_s0.json` | none | binder checkpoint ignored, result JSON tracked | PR7b binder transfer to PR4 grid. Exact 0.722. Seen-template cells C1/C3 reach 1.000, but held-out-template cells fail: C2 0.448, C4 0.438. This keeps the software compiler as the strongest path for PR8 span/long-context isolation. |
| `slm-qwen25-0.5b-instruct-pr8-multispan-oracle-s0` | completed / boundary | CLI: `scripts/slm_policy_vector.py --dataset-kind multi_span_grid --steps 300 --eval-every 100 --train-pairs 512 --eval-pairs 8 --eval-controls correct constant_policy invert_policy --permission-rail oracle --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --lr 2e-4 --fail-on-truncation` | `results/slm/qwen25_0_5b_instruct_pr8_multispan_oracle_s0.json` | none | permission embedding checkpoint ignored, result JSON tracked | PR8 first span-scaling rung. Primary candidate plus distractor candidate, software oracle compiler. Exact 0.965; C1 1.000, C2 0.938, C3 1.000, C4 0.917. Constant-policy 0.444, invert-policy 0.003. Causal but not clean; PR9/PR10 are gated behind PR8b multi-span template fix. |
| `slm-qwen25-0.5b-instruct-pr8b-multispan-oracle-step200-s0` | completed / unblocks PR9 | CLI: `scripts/slm_policy_vector.py --dataset-kind multi_span_grid --steps 200 --eval-every 50 --train-pairs 512 --eval-pairs 32 --eval-use-heldout-values --eval-controls correct constant_policy invert_policy --permission-rail oracle --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --batch-size 16 --eval-batch-size 8 --max-new-tokens 16 --lr 2e-4 --fail-on-truncation` | `results/slm/qwen25_0_5b_instruct_pr8b_multispan_oracle_step200_s0.json` | none | permission embedding checkpoint ignored, result JSON tracked | PR8b fixed-step multi-span retest with larger held-out-value eval and error-type diagnostics. Exact 0.989; C1 1.000, C2 0.982, C3 1.000, C4 0.969. Constant-policy 0.444, invert-policy 0.002. Distractor error rate 0.000, so PR8's boundary was not wrong-span bleed. PR9 scale/architecture replication is unblocked; PR10 remains gated behind PR9. |
| `slm-qwen25-1.5b-instruct-pr9-multispan-oracle-step200-s0` | completed / scale-boundary | CLI: `scripts/slm_policy_vector.py --model Qwen/Qwen2.5-1.5B-Instruct --dataset-kind multi_span_grid --steps 200 --eval-every 100 --train-pairs 512 --eval-pairs 8 --eval-use-heldout-values --eval-controls correct constant_policy invert_policy --permission-rail oracle --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --batch-size 8 --eval-batch-size 4 --max-new-tokens 16 --lr 2e-4 --fail-on-truncation` | `results/slm/qwen25_1_5b_instruct_pr9_multispan_oracle_step200_s0.json` | none | permission embedding checkpoint ignored, result JSON tracked | PR9 first 1.5B scale replication. Same rail mechanism, 4,608 trainable params. Exact 0.948; C1 0.979, C2 0.917, C3 0.979, C4 0.917. Constant-policy 0.444, invert-policy 0.017, distractor error 0.000. Causal but below the PR8b all-cell gate. |
| `slm-qwen25-1.5b-instruct-pr9b-multispan-oracle-samplematched-s0` | early-killed / diagnostic | CLI: `scripts/slm_policy_vector.py --model Qwen/Qwen2.5-1.5B-Instruct --dataset-kind multi_span_grid --steps 400 --eval-every 100 --train-pairs 512 --eval-pairs 8 --eval-use-heldout-values --eval-controls correct constant_policy invert_policy --permission-rail oracle --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --batch-size 8 --eval-batch-size 4 --max-new-tokens 16 --lr 2e-4 --fail-on-truncation` | `results/slm/qwen25_1_5b_instruct_pr9b_multispan_oracle_samplematched_s0.partial.json` | none | partial result tracked | PR9b sample-exposure diagnostic. Step 200 exactly matches PR9. Step 300 regresses to exact 0.882; C2 0.766, C4 0.760, invert-policy 0.000, distractor error 0.000, other/format error 0.118. More sampled examples do not fix 1.5B; it overfits held-out carrier phrasing. |
| `slm-qwen25-1.5b-instruct-pr9c-multispan-value-delimited-s0` | completed / unblocks PR10 | CLI: `scripts/slm_policy_vector.py --model Qwen/Qwen2.5-1.5B-Instruct --dataset-kind multi_span_grid --template-family value_delimited --steps 200 --eval-every 100 --train-pairs 512 --eval-pairs 8 --eval-use-heldout-values --eval-controls correct constant_policy invert_policy --permission-rail oracle --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --batch-size 8 --eval-batch-size 4 --max-new-tokens 16 --lr 2e-4 --fail-on-truncation` | `results/slm/qwen25_1_5b_instruct_pr9c_multispan_value_delimited_s0.json` | none | permission embedding checkpoints ignored, result JSON tracked | PR9c value-boundary fix. Same 1.5B rail but candidate values are explicitly delimited (`VALUE=`, brackets, XML-like tags). Exact 1.000 on all cells and all OPEN/DECLINE primitive buckets. Constant-policy 0.444, invert-policy 0.000, all error rates 0.000. Confirms PR9 failure was output extraction ambiguity, not policy lookup or span-binding failure. |
| `slm-qwen25-1.5b-instruct-pr10-risk-value-delimited-s0` | completed / passes synthetic risk rung | Train CLI: `scripts/slm_policy_vector.py --model Qwen/Qwen2.5-1.5B-Instruct --dataset-kind risk_multispan_grid --template-family value_delimited --risk-rail embedding --steps 200 --eval-every 100 --train-pairs 512 --eval-pairs 2 --eval-controls correct --permission-rail oracle --no-source-embeddings --no-operation-embeddings --no-policy-bit-embeddings --batch-size 8 --eval-batch-size 16 --max-new-tokens 16 --lr 2e-4 --fail-on-truncation`; all-control eval reloads best adapter with `--steps 0 --eval-pairs 4 --eval-controls correct constant_policy invert_policy constant_risk invert_risk` | `results/slm/qwen25_1_5b_instruct_pr10_risk_value_delimited_s0.json`, `results/slm/qwen25_1_5b_instruct_pr10_risk_value_delimited_train_s0.json` | none | tiny permission/risk checkpoints ignored, result JSON tracked | PR10 adds a separate hidden risk rail (`SAFE/SENSITIVE/HARMFUL`) on top of the PR9c value-delimited permission stack. Trains 10,752 params. Correct exact 0.995 over 864 rows; C1 0.997, C2 0.997, C3 0.993, C4 0.993; risk_allow 0.988, risk_refuse 1.000, permission_decline 1.000; distractor error 0.000. Traps: constant-policy 0.444, invert-policy 0.005, constant-risk 0.773, invert-risk 0.444. |
| `slm-qwen25-0.5b-instruct-hidden-role-layer-slices-s0` | deferred | early `--lora-layer-max 7`, mid `--lora-layer-min 8 --lora-layer-max 15`, late `--lora-layer-min 16` | `results/slm/qwen25_0_5b_instruct_hidden_role_{early,mid,late}_s0.json` | planned | adapter checkpoints ignored, result JSON tracked | Deferred because role-only succeeds; layer-sliced LoRA no longer tests a necessary mechanism for this setup. |
| `slm-qwen25-0.5b-instruct-hidden-role-semantic-triad-s0` | completed via role-only | hidden-role gated task using all gate kinds by default, including semantic tool/place/material gates | `results/slm/qwen25_0_5b_instruct_hidden_role_roleonly_s0.json` | `d8icto6q` | result JSON tracked | Template-shortcut kill test passed in the stronger role-only setting: correct 0.996, trained constant 0.438, eval-swap 0.000. |
| `slm-qwen25-0.5b-instruct-hidden-role-subspace-s0` | completed | `scripts/analyze_slm_adapter_subspace.py` over the saved correct-role adapter | `results/slm/qwen25_0_5b_instruct_hidden_role_gated_correct_subspace_s0.json` | local | JSON tracked | Learned LoRA deltas are low stable-rank (weighted mean 1.91) and role embedding active rows have stable-rank 2.89, but this simple overlap metric finds near-zero cosine with the base-to-instruct delta and capture near random-baseline scale. |
| `lrs1-qwen25-srank-cross-check` | completed external | `/home/debanjan/Code/Research/lean-mining/cross-check/preregistry/lrs1_srank_scaling_qwen25_2026-05-17` | `/home/debanjan/Code/Research/lean-mining/artifacts/lazy-rudder-srank/lrs1_srank_scaling_qwen25_2026-05-17.json`; checkpoints offloaded to `https://huggingface.co/d3banjan/lrs1-srank-qwen25-checkpoints` | lean-mining commits `845956c`, `91aa4c1` | external JSON tracked in lean-mining; local checkpoints deleted after HF verification | Plain HH-RLHF DPO LoRA geometry baseline. Qwen2.5 attn_qkv_srank: 0.5B 3.435, 1.5B 3.963, 3B 3.939. Replicates lazy-rudder flat floor across GPT-NeoX -> Llama-style Qwen. |
| `toy-rope-pos-add-role-s0` | deferred | pending script option | `results/toy/rope_pos_add_role_s0.json` | planned | result JSON only | Deferred; no script option exists yet, and the SLM additive hidden-role triad is more directly relevant. |
| `toy-rope-role-rotation-s0` | deferred | pending script option | `results/toy/rope_role_rotation_s0.json` | planned | result JSON only | Deferred; no script option exists yet, and the current paper matrix already has the stronger SmolLM2 rotational negative. |

Operational caveat: Qwen2.5 emits a Transformers warning that sliding-window
attention is enabled but not implemented for `sdpa`. Local config inspection on
2026-05-17 showed `use_sliding_window=False`, `sliding_window=32768`, and the
hidden-role prompts are only ~101-118 tokens. The warning is therefore not
expected to affect these short-context runs, but it should be rechecked before
any long-context Qwen eval.
...
- `slm-qwen25-0.5b-instruct-hidden-role-gated-correct-s0`: `https://wandb.ai/d3banjan/rope-provenance/runs/hdjhcz4q`
- `slm-qwen25-0.5b-instruct-hidden-role-gated-constant-s0`: `https://wandb.ai/d3banjan/rope-provenance/runs/rldphw35`
- `slm-qwen25-0.5b-instruct-hidden-role-gated-evalswap-s0`: `https://wandb.ai/d3banjan/rope-provenance/runs/u9uht6a8`
- `slm-qwen25-0.5b-instruct-hidden-role-gated-correct-strict-eval-s0`: `https://wandb.ai/d3banjan/rope-provenance/runs/qu1d8i4g`
- `slm-qwen25-0.5b-instruct-hidden-role-gated-constant-strict-eval-s0`: `https://wandb.ai/d3banjan/rope-provenance/runs/e54lk3vo`
- `slm-qwen25-0.5b-instruct-hidden-role-gated-evalswap-strict-eval-s0`: `https://wandb.ai/d3banjan/rope-provenance/runs/1w49y3cg`
- `slm-qwen25-0.5b-instruct-hidden-role-roleonly-s0`: `https://wandb.ai/d3banjan/rope-provenance/runs/d8icto6q`
- `slm-qwen25-0.5b-instruct-hidden-role-roleonly-constant-eval-s0`: `https://wandb.ai/d3banjan/rope-provenance/runs/f34o4v4u`
- `slm-qwen25-0.5b-instruct-hidden-role-roleonly-evalswap-s0`: `https://wandb.ai/d3banjan/rope-provenance/runs/s0t5jxbt`
- `slm-qwen25-0.5b-instruct-hidden-role-roleonly-constant-train-s0`: `https://wandb.ai/d3banjan/rope-provenance/runs/n5i8ped4`
- `slm-qwen25-0.5b-base-hidden-role-roleonly-smoke-s0`: `https://wandb.ai/d3banjan/rope-provenance/runs/q3eoikyp`
