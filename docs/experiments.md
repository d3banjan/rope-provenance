...
| `slm-qwen25-0.5b-instruct-hidden-role-roleonly-s0` | planned | same hidden-role gated setup with `--input-role-embeddings --lora-rank 0` | `results/slm/qwen25_0_5b_instruct_hidden_role_roleonly_s0.json` | planned | role embedding checkpoint ignored, result JSON tracked | Tests whether the frozen instruct model already exposes a role-control direction. If it succeeds, LoRA is not necessary for this setup. |
| `slm-qwen25-0.5b-instruct-hidden-role-layer-slices-s0` | planned | early `--lora-layer-max 7`, mid `--lora-layer-min 8 --lora-layer-max 15`, late `--lora-layer-min 16` | `results/slm/qwen25_0_5b_instruct_hidden_role_{early,mid,late}_s0.json` | planned | adapter checkpoints ignored, result JSON tracked | Minimal layer-locality test for the low-rank authority-rudder thesis. |
| `slm-qwen25-0.5b-instruct-hidden-role-semantic-triad-s0` | planned | hidden-role gated triad using all gate kinds by default, including semantic tool/place/material gates | `results/slm/qwen25_0_5b_instruct_hidden_role_semantic_{correct,constant,evalswap}_s0.json` | planned | adapter checkpoints ignored, result JSON tracked | Template-shortcut kill test with disjoint semantic train/eval clue vocabularies and strict scoring. |
