...
| `slm-qwen25-0.5b-instruct-hidden-role-gated-constant-s0` | planned | same as correct hidden-role run but `--role-control constant` | `results/slm/qwen25_0_5b_instruct_hidden_role_gated_constant_s0.json` | planned | adapter checkpoint ignored, result JSON tracked | Required text-only contradiction control. Expected to cap near 0.50 because paired examples have identical visible prompts with opposite labels; <=0.55 supports role-channel causality if correct passes. |
