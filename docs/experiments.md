...
| `slm-qwen25-0.5b-instruct-gated-probe-s0` | planned | same script/model with `--prompt-format chat --steps 500 --batch-size 16 --eval-batch-size 16 --lora-rank 8 --lr 2e-4` | `results/slm/qwen25_0_5b_instruct_gate_pretrain_s0.json` | planned | adapter checkpoint ignored, result JSON tracked | Confounded fine-tune probe only. Passing threshold: exact-match >=0.50. If it works, the task is reachable after instruction post-training; if it overfits/collapses, redesign the generator/loss before hidden-role work. |
